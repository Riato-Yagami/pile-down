...
static func resize_dust_distribution(game: GameManager) -> void:
	# Viewport and container notifications can describe the same layout pass.
	# Keep only one pending pass, using the latest dimensions before drawing.
	if game._viewport_effects_resize_pending:
		return
	game._viewport_effects_resize_pending = true
	game.call_deferred("_flush_viewport_effects_resize")


static func flush_viewport_effects_resize(game: GameManager) -> void:
	...
	if is_instance_valid(game.background_effects):
		game.background_effects.resize_to_viewport()
	if is_instance_valid(game.background_manager):
		game.background_manager.resize_to_viewport()
	if is_instance_valid(game.dust_pool):
		game.dust_pool.resize_to_viewport(dust_particle_target_count(game))
	game._viewport_effects_resize_pending = false
...
static func setup_save_dialogs(game: GameManager) -> void:
	...
	# Explicit line breaks keep the minimum height stable before the window opens.
	...
	game.get_viewport().size_changed.connect(
		game.delete_save_confirmation.move_to_center, CONNECT_DEFERRED
	)
